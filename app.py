...
3. JOIN STYLE — use ONE style only: the Oracle LEGACY comma-join style. List every table comma-separated in the FROM clause and put EVERY join condition in the WHERE clause, with the (+) operator on the optional (outer) side: AND Header.VENDOR_ID = Supplier.VENDOR_ID(+). NEVER use ANSI join keywords (JOIN, INNER JOIN, LEFT/RIGHT/FULL OUTER JOIN, ON, USING). The (+) operator is ILLEGAL inside an ANSI ON clause, and mixing the two styles causes ORA-00907 (missing right parenthesis). Every join recipe below is written in this legacy (+) style — copy it verbatim into the WHERE clause; do NOT rewrite it as JOIN ... ON.
...
33. NEVER MIX JOIN SYNTAX — this is a top cause of ORA-00907 (missing right parenthesis) in this app. Do NOT combine ANSI joins (LEFT JOIN ... ON) with the (+) operator. The (+) operator may appear ONLY in the WHERE clause of a legacy comma-join query — never inside an ON clause. This applies especially to date-effective BETWEEN predicates: write them in the WHERE clause as AND TRUNC(SYSDATE) BETWEEN Loc.EFFECTIVE_START_DATE(+) AND Loc.EFFECTIVE_END_DATE(+) — NOT inside a LEFT JOIN ... ON clause. If you ever write the word JOIN, stop and rewrite the ENTIRE query in legacy comma-join style with all tables comma-separated in FROM and all conditions (including every (+)) in WHERE. A single query must be 100% legacy style or it will fail to parse.
